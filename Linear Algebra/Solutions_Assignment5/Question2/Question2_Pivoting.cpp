/*============================================================================
Program     : Gauss Elimination Method with Pivoting
...
int main()
{
    const int n=3; // Save number of eqautions. Or size of Matrix A
    ...
    //Choosing Pivot Element(Partial Pivoting) + Gauss Elimination
    for (i=0;i<n-1;i++)for(j=i+1;j<n;j++){
               //Pivotisation
               for(int m=j;m<n;m++)if(abs(A[i][i])<abs(A[m][i]))for(int c=0;c<n+1;c++){double temp=A[i][c]; A[i][c]=A[m][c]; A[m][c]=temp;}
               //Elimination //3 Floating Point Ops each for div, mul and subtraction.
